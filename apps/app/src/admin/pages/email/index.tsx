...
function InstanceEmailForm({ config }: { config: IFormattedInstanceConfiguration }) {
  ...
  const emailFormFields: TControllerInputFormField[] = [
    { key: "EMAIL_HOST", type: "text", label: "Host", placeholder: "email.google.com", error: Boolean(errors.EMAIL_HOST), required: true },
    { key: "EMAIL_PORT", type: "text", label: "Port", placeholder: "8080", error: Boolean(errors.EMAIL_PORT), required: true },
    {
      key: "EMAIL_FROM",
      type: "text",
      label: "Sender's email address",
      description: "This is the email address your users will see when getting emails from this instance. You will need to verify this address.",
  ...
  const optionalEmailFormFields: TControllerInputFormField[] = [
    { key: "EMAIL_HOST_USER", type: "text", label: "Username", placeholder: "getitdone@projectplane.so", error: Boolean(errors.EMAIL_HOST_USER), required: false },
    { key: "EMAIL_HOST_PASSWORD", type: "password", label: "Password", placeholder: "Password", error: Boolean(errors.EMAIL_HOST_PASSWORD), required: false },
  ];

  const onSubmit = async (formData: EmailFormValues) => {
    const payload: Partial<EmailFormValues> = { ...formData, ENABLE_SMTP: "1" };
    await updateInstanceConfigurations(payload)
      .then(() => setToast({ type: TOAST_TYPE.SUCCESS, title: "Success", message: "Email Settings updated successfully" }))
  ...
  const handleEmailSecurityChange = (key: TEmailSecurityKeys) => {
    if (key === "EMAIL_USE_SSL") { setValue("EMAIL_USE_TLS", "0"); setValue("EMAIL_USE_SSL", "1"); }
    if (key === "EMAIL_USE_TLS") { setValue("EMAIL_USE_TLS", "1"); setValue("EMAIL_USE_SSL", "0"); }
    if (key === "NONE") { setValue("EMAIL_USE_TLS", "0"); setValue("EMAIL_USE_SSL", "0"); }
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <div className="grid-col grid w-full max-w-4xl grid-cols-1 items-start justify-between gap-10 lg:grid-cols-2">
          ...
          <div className="flex flex-col gap-1">
            ...
            <CustomSelect value={emailSecurityKey} label={EMAIL_SECURITY_OPTIONS[emailSecurityKey]} onChange={handleEmailSecurityChange} buttonClassName="rounded-md border-subtle" input>
              {Object.entries(EMAIL_SECURITY_OPTIONS).map(([key, value]) => (
                <CustomSelect.Option key={key} value={key} className="w-full">{value}</CustomSelect.Option>
              ))}
            </CustomSelect>
          </div>
        </div>
          ...
          <div className="flex w-full max-w-xl flex-col gap-y-10 px-1">
            <div className="mr-8 flex items-center gap-10 pt-4">
              <div className="grow">
                ...
                <div className="text-11 font-regular text-tertiary">This is optional, but we recommend setting up a username and a password for your SMTP server.</div>
              </div>
            </div>
          </div>
      ...
      <div className="flex max-w-4xl items-center py-1 gap-4">
        <Button variant="primary" size="lg" onClick={handleSubmit(onSubmit)} loading={isSubmitting} disabled={!isValid || !isDirty}>
          {isSubmitting ? "Saving" : "Save changes"}
        </Button>
        <Button variant="secondary" size="lg" onClick={() => setIsSendTestEmailModalOpen(true)} loading={isSubmitting} disabled={!isValid}>
          Send test email
        </Button>
      </div>
    </div>
...
const InstanceEmailPage = observer(function InstanceEmailPage() {
  ...
  const handleToggle = async () => {
    if (isSMTPEnabled) {
      ...
      try {
        ...
        setToast({ title: "Email feature disabled", message: "Email feature has been disabled", type: TOAST_TYPE.SUCCESS });
      } catch (_error) {
        setToast({ title: "Error disabling email", message: "Failed to disable email feature. Please try again.", type: TOAST_TYPE.ERROR });
      } finally {
  ...
  useEffect(() => {
    if (formattedConfig) setIsSMTPEnabled(formattedConfig.ENABLE_SMTP === "1");
  }, [formattedConfig]);
  ...
  return (
    <PageWrapper
      header={{
        ...
        actions: isLoading ? (
          <Loader><Loader.Item width="24px" height="16px" className="rounded-full" /></Loader>
        ) : (
