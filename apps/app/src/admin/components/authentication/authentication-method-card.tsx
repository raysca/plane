...
export function AuthenticationMethodCard(props: Props) {
  ...
  return (
    <div className={cn("w-full flex items-center gap-14 rounded-lg bg-layer-2", { "px-4 py-3 border border-subtle": withBorder })}>
      <div className={cn("flex grow items-center gap-4", { "opacity-50": unavailable })}>
        ...
        <div className="grow">
          <div className={cn("font-medium leading-5 text-primary", { "text-13": withBorder, "text-18": !withBorder })}>{name}</div>
          <div className={cn("font-regular leading-5 text-tertiary", { "text-11": withBorder, "text-13": !withBorder })}>{description}</div>
        </div>
      </div>
