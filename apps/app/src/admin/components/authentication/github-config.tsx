    ...
    <>
      {isGithubConfigured ? (
        <div className="flex items-center gap-4">
          <Link to="/authentication/github" className={cn(getButtonStyling("link", "base"), "font-medium")}>Edit</Link>
          <ToggleSwitch
            value={Boolean(parseInt(enableGithubConfig))}
            onChange={() => {
              const newVal = Boolean(parseInt(enableGithubConfig)) === true ? "0" : "1";
              updateConfig("IS_GITHUB_ENABLED", newVal);
            }}
            size="sm"
            disabled={disabled}
          />
        ...
        <Link to="/authentication/github" className={cn(getButtonStyling("secondary", "base"), "text-tertiary")}>
