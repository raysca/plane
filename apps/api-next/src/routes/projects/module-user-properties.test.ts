...
// GET
app.get("/api/workspaces/:slug/projects/:projectId/modules/:moduleId/user-properties/", async (c) => {
  ...
  const foundModule = await db.query.modules.findFirst({
    where: and(eq(moduleSchema.modules.id, moduleId), eq(moduleSchema.modules.projectId, projectId)),
  });
  if (!foundModule) return c.json({ detail: "Module not found." }, 404);

  let props = await db.query.moduleUserProperties.findFirst({
    where: and(
      eq(moduleSchema.moduleUserProperties.moduleId, moduleId),
      eq(moduleSchema.moduleUserProperties.userId, user.id)
    ),
  });

  if (!props) {
    const [created] = await db.insert(moduleSchema.moduleUserProperties).values({
      moduleId,
      projectId,
...
// PATCH
app.patch("/api/workspaces/:slug/projects/:projectId/modules/:moduleId/user-properties/", async (c) => {
  const user = getCurrentUser(c);
  if (!user) return c.json({ detail: "Authentication required." }, 401);
  ...
  await db.update(moduleSchema.moduleUserProperties)
    .set(updateData)
    .where(eq(moduleSchema.moduleUserProperties.id, props!.id));
...
const url = (moduleId: string) =>
  `/api/workspaces/${workspace.slug}/projects/${project.id}/modules/${moduleId}/user-properties/`;

describe("Module User Properties - GET", () => {
  test("auto-creates default properties on first access", async () => {
    const res = await app.request(url(mod.id), {
      headers: { "x-test-user-id": testUser.id },
    });
    expect(res.status).toBe(200);
    const data = await res.json();

    expect(data.module).toBe(mod.id);
  ...
  test("returns same properties on subsequent access", async () => {
    const res1 = await app.request(url(mod.id), {
      headers: { "x-test-user-id": testUser.id },
    });
    const data1 = await res1.json();

    const res2 = await app.request(url(mod.id), {
      headers: { "x-test-user-id": testUser.id },
    });
...
describe("Module User Properties - PATCH", () => {
  ...
  test("auto-creates properties on PATCH if not exists", async () => {
    ...
    const res = await app.request(url(newModule.id), {
      method: "PATCH",
      headers: { "Content-Type": "application/json", "x-test-user-id": testUser.id },
      body: JSON.stringify({
    ...
    expect(res.status).toBe(200);
    const data = await res.json();
    expect(data.module).toBe(newModule.id);
